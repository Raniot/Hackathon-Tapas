...
def minDistanceBetweenLineAndPoint(lat1, lon1, lat2, lon2, lat3, lon3):
...
here = (56.1723027, 10.1877547)

point1 = (56.171977, 10.187193)
point2 = (56.171944, 10.187414)
...
while True:
    try:
        ...
        elif loglines.find('$GNGLL') >=0 and len(loglines) == 51:
            print(loglines)
            latitude = float(loglines[7:9]) + float(loglines[9:17])/60
            longitude = float(loglines[20:23]) + float(loglines[23:31])/60
            pos = (latitude, longitude)
            print(f"Distance to destination in cm: {geodesic(pos, point2).meters*100}")
            minDistanceBetweenLineAndPoint(point1[0], point1[1], point2[0], point2[1], latitude, longitude)
    except:
